$(document).ready(function () {
  ...
  const aniState = ((e) => {
    ...
    $('.animate__animated').each(function () {
      if (this.getBoundingClientRect().top < _h * 0.9) {
        this.classList.remove("ani_out");
        this.classList.add("ani");
        this.querySelectorAll(".swiper-slide-active video").forEach(el => {
          if (this.parentElement.classList.contains('pause') !== false) {
            el.currentTime = 0;
            el.play();
          }
        });
  ...
  $(`[data-swiper-type="thumb"]`).each(function () {
    ...
    const _autoheight = this.getAttribute("data-swiper-autoHeight") || false;
    ...
    _list_el._swiper = new Swiper(_list_el, {
      ...
      on: {
        slideChange: function (swiper) {
          ...
          // 페이징 시 Swiper 컨테이너 상단으로 이동
          window.scrollTo({
            top: this.el.getBoundingClientRect().top + window.pageYOffset - 500,
            behavior: 'smooth',
          });
        }
      },
    });
    $(_list_el).on("click", "button", function () {
      _list_el._swiper.updateAutoHeight();
    });

    // MutationObserver로 슬라이드 콘텐츠 변경 감지 및 높이 갱신
    const observer = new MutationObserver(() => {
      _list_el._swiper.updateAutoHeight();
    });

    observer.observe(_list_el.querySelector(".swiper-wrapper"), {
      childList: true,
      subtree: true,
    });
  });
  ...
  /* 히터 방식 슬라이드 */
  var swiper2 = new Swiper('.heater-method.swiper', {
    observer: true,
    observeParents: true,
    speed: 1000,
    nested: true,
    pagination: {
      el: '.heater-method.swiper .pagi',
      clickable: true
    },
  });

  /* 히터 방식 슬라이드 */
  $('.dep2-tab li').click(function () {
    var idx = $(this).index();
    $(this).addClass('on').siblings().removeClass('on');
    $(this).parent().siblings('.vid-box').children('video').hide().eq(idx).css('display', 'block');
  });
});