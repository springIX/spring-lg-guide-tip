(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(window).on("scroll", function () {
        ...
        if (isMobile) {
          ...
          if (currentURL.startsWith("https://wwwstg.lge.co.kr/")) {
            ...
            if ($("header.header").length) {
              topGap =
                $("header").height() + $(".hello-bar-wrap.hello-app").height();
            } else {
              topGap = 0;
            }
          } else {
      ...
      $("#INTERVIEW .interview-list .interview-info").css(
        "transform",
        "translateY(" + (topGap + $nav.height()) + "px)",
      ); // 'top' 대신 'transform: translateY' 사용
      lastScrollTop = st;
    });
